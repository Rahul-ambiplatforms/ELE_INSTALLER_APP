...
exports.getPsDetails = async (req, res, next) => {
    try {
        ...
        const details = await Booth.aggregate([
            { $match: { state: state, district: district, assemblyName: assemblyName } }, // Match documents with the given state, district, and assemblyName
        ...
        const details = await Booth.aggregate([
            {
                $match: {
                    ...
                    psNo: typeof psNo === 'string' && psNo.includes('_') ? psNo : Math.floor(psNo)
                }
            },
...
exports.updateStr = async () => {
    ...
    try {
        ...
        const documentsToUpdate = await collection.find({ psNo: { $exists: true, $type: 'int' } }).toArray();
...
exports.getDashboardDetails = async (req, res, next) => {
    try {
        ...
        res.status(200).json({
            success: true,
            data: { totalCameras, installedCameras, totalInstallers, totalDistrictManager, totalLiveCamera, uniqueState, totalOfflineCamera, dataByState },
        });
    } catch (error) {
        res.status(500).json({
            success: false,
...
exports.getAssemblyData = async (req, res, next) => {
    try {
        ...
        dataByAssembly.sort((a, b) => {
            const ratioA = a.totalCameras === 0 ? 0 : a.totalLiveCamera / a.totalCameras;
            const ratioB = b.totalCameras === 0 ? 0 : b.totalLiveCamera / b.totalCameras;
...
exports.trackLiveLatLong = async (req, res, next) => {
    try {
        // console.log(req.body)
        // let deviceId = req.query.deviceId;
        const { latitude, longitude, personName, personMobile, date, time, state, formatted_address, formatted_address1, formatted_address2 } = req.body
        // console.log("deviceId:", deviceId); // For debugging
...
exports.getElectionUser = async (req, res, next) => {
    try {
        ...
        const lastHour = await electionUser.countDocuments({ date: formattedDate, time: { $gte: time } });
...
exports.getElectionUserChart = async (req, res, next) => {
    try {
        ...
        // Count documents within the last hour
        const currentHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: currentTisplit } });
        const lastHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: timesplit } });
        const twoHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time2split } });
        const threeHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time3split } });
        const fourHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time4split } });
        const fiveHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time5split } });
        const sixHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time6split } });
        const sevenHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time7split } });
        const eightHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time8split } });
        const nineHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time9split } });
        const tenHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time10split } });
        const elevenHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time11split } });
        const twelveHour = await electionUser.countDocuments({ date: formattedDate, time: { $regex: time12split } });
...
exports.getElectionCameraChart = async (req, res, next) => {
    try {
        const currentTime = new Date();
        const formattedDate = currentTime.toLocaleDateString('en-GB', { timeZone: 'Asia/Kolkata' });
        // const time = getTimeOneHourAgo(); // Get the time one hour ago
        const current = new Date();
        const oneHourAgo = new Date();
        const twoHourAgo = new Date();
...
exports.getSixDistrictDataBihar = async (req, res, next) => {
    try {
        let districts = ['sitamarhi', 'muzaffarpur', 'saran', 'vaishali', 'madhubani', 'darbhanga', 'patna'];
                        ...
                        $arrayElemAt: [
                            {
                                $filter: {
                                    input: districts,
                                    as: "district",
                                    cond: { $regexMatch: { input: "$formatted_address", regex: { $concat: ["(?i)", "$$district"] } } }
                                }
                            },
                            0
                        ]
